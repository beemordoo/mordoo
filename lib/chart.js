...
async function getBirthChart(dateStr) {
    ...
    await Promise.allSettled(batch.map(async (p) => {
      ...
      try {
        ...
        if (lon !== null) {
          ...
          console.error(`[chart] ${p}: parseJPLLongitude returned null for ${dateStr}`);
        }
      } catch (e) {
        failed.push({ planet: p, reason: e.message || 'unknown' });
        console.error(`[chart] ${p}: ${e.message} (${dateStr})`);
      }
    }));
  };
  ...
  console.log(`[chart] fetched: ${Object.keys(results).join(', ')} (${Object.keys(results).length}/7)`);
  if (failed.length > 0) {
    console.error(`[chart] PARTIAL chart for ${dateStr}: ${failed.length}/7 planets failed after rescue:`,
