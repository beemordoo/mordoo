...
async function fetchPlanetPosition(jplId, dateStr) {
  ...
  try {
    ...
    // Network/timeout/HTTP error — diagnostic captures timing + HTTP status
    // so we can tell timeout from connection-refused from 5xx-from-JPL.
    return {
      ok: false,
      reason: e.message === 'JPL timeout' ? 'fetch_timeout' : 'fetch_error',
      diagnostic: {
        elapsedMs: elapsed,
        httpStatus,
  ...
  if (!parseResult.ok) {
    // Fold fetch timing into parser diagnostic so failure logs have full picture
